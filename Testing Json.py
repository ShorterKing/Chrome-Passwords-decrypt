...
parser = argparse.ArgumentParser(description='Chrome Data Decryptor')
parser.add_argument('--json', action='store_true', help='Output results in JSON format')
args = parser.parse_args()

# Check for admin privileges
if not is_admin():
    print("This script needs to run as administrator. Please restart with admin privileges.")
    sys.exit(1)
...
if args.json:
    ...
    for c in decrypted_cookies:
        print(c["host_key"], c["name"], c["value"])

    # print decrypted passwords
    print("\nDecrypted Passwords:")
    for p in decrypted_passwords:
        print(p["origin_url"], p["username"], p["password"])
